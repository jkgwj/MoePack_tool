...
int MoeUnpack::unpack_log_level = 1;
...
UniquePtr_uChar MoeUnpack::_decrypt_chunked(unsigned char* data, int& size,
                                             const MoeHeader& header,
                                             const std::string& key) {
    if (sodium_init() == -1) {
...
MoeUnpack::_UnpackResult MoeUnpack::_unpack_process_file(const std::string& in_put, const std::string& _key) {
    ...
    try {
        MoeUnpack::unpack_moe_header(reinterpret_cast<unsigned char*>(&moe_header), moe_header);
